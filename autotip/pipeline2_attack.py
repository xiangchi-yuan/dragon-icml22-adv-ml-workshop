...
import config_citeseer as config

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Attacking GNN models in pipeline.')
    # Dataset settings
    parser.add_argument("--dataset", type=str, default="grb-citeseer")
    ...
    parser.add_argument("--n_attack", type=int, default=None)
    parser.add_argument("--n_inject_ratio", type=float, default=None)
    parser.add_argument("--n_inject_max_base", type=int, default=150)
    parser.add_argument("--n_inject_max", type=int, default=100)
    ...
    # if args.n_inject_ratio is not None:
        # args.n_inject_max = int(dataset.num_nodes * args.n_inject_ratio)
        # args.n_edge_max = int(dataset.num_edges / dataset.num_nodes)
        # args.n_inject_max = args.n_inject_max * args.n_inject_ratio
    ...
    print("{} attack..........".format(args.n_inject_ratio))

    for i in range(1, 6):
        args.n_inject_max = args.n_inject_max_base * i
        print("    Number of injection: {}".format(args.n_inject_max))
        print("    Number of maximum edges for each injected nodes: {}".format(args.n_edge_max))
        attack = config.build_attack(attack_name=args.attack,
                                     device=device,
                                     args=args,
                                     mode=args.attack_mode)

        model_name = "gcn_ln"
        print("{} vs. {}..........".format(args.attack, model_name))
        ...
        save_dir = os.path.join(args.save_dir, args.attack + "_vs_" + model_name, str(i))
