...
edge_index = adj2edge_index(adj)
data_original = Data(x=features, edge_index=edge_index)
...
train_data, val_data, test_data = T.RandomLinkSplit(num_val=0.05, num_test=0.2,
                                                    is_undirected=True,
                                                    split_labels=True,
...
for i in range(1, 6):
    if i != 0:
        ...
        adj_attacked = adj
        features_attacked = features.to(device)
    ...
    CoraNet = to_networkx(data_attacked)
    CoraNet = CoraNet.to_undirected()
    edge_atk = [(u, v) for (u, v, d) in CoraNet.edges(data=True) if (u > 2680 or v > 2680) and (u % 5 == 0 and v % 5 == 0)]
    edge_ori = [(u, v) for (u, v, d) in CoraNet.edges(data=True) if (u <= 2680 and v <= 2680) and (u % 5 == 0 and v % 5 == 0)]
    # node_atk = [n for (n) in CoraNet.nodes(data=True) if int(n) > 2680]
    # node_ori = [n for (n) in CoraNet.nodes(data=True) if int(n) <= 2680]
    CoraNet.remove_edges_from(CoraNet.edges())
    CoraNet.add_edges_from(edge_atk)
    CoraNet.add_edges_from(edge_ori)
    CoraNet.remove_nodes_from(list(nx.isolates(CoraNet)))
    pos = nx.spring_layout(CoraNet, k=0.1)
    # nx.draw_networkx_nodes(CoraNet, pos, nodelist=node_atk, width=0.05, edge_color="black")
    # nx.draw_networkx_nodes(CoraNet, pos, nodelist=node_atk, width=0.05, edge_color="red")
    nx.draw_networkx_edges(CoraNet, pos, edgelist=edge_ori, width=0.1,alpha=0.2,  edge_color="black")
    ...
    full_edge = torch.cat((val_data.edge_index.to(device),train_data.edge_index, edge_rec), 1)
    attr = torch.ones(full_edge[0].shape[0])
    adj_rec = build_adj(attr, full_edge.to('cpu'), adj_type='csr')

    #==========================
    data_rec = Data(x=features, edge_index=full_edge)
    clnNet = to_networkx(data_rec)
    clnNet = clnNet.to_undirected()
    edge_recatk = [(u, v) for (u, v, d) in clnNet.edges(data=True) if (u > 2680 or v > 2680) and (u % 5 == 0 and v % 5 == 0)]
    edge_recori = [(u, v) for (u, v, d) in clnNet.edges(data=True) if (u <= 2680 and v <= 2680) and (u % 5 == 0 and v % 5 == 0)]

    clnNet.remove_edges_from(clnNet.edges())
    clnNet.add_edges_from(edge_recatk)
    clnNet.add_edges_from(edge_ori)
    clnNet.remove_nodes_from(list(nx.isolates(clnNet)))
    pos = nx.spring_layout(CoraNet, k=0.1)
    nx.draw_networkx_nodes(CoraNet, pos,node_shape="s", node_size=0.1)
