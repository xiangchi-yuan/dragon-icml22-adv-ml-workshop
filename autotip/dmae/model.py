...
class MaskGAE(nn.Module):
    ...
    @torch.no_grad()
    def edge_pred(self, z, pos_edge_index, batch_size=2 ** 16):

        return self.batch_predict(z, pos_edge_index)

    @torch.no_grad()
    def graph_reconstruct(self, z, pos_edges, batch_size=2 ** 16):
        preds = []
        ...
        for i in range(pred.size):
            if pred[i] > 0.5:
                reconstruct_index += [i]
        reconstruct_edges = pos_edges[:, reconstruct_index]
